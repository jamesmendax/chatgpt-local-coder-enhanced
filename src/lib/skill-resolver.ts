...
export interface ResolveSkillsInput {
  ...
  codexHome: string;
  /**
   * Read-only host Codex home used only to discover OpenAI's bundled
   * Computer Use Skill.  This is deliberately separate from `codexHome`,
   * which may be an account-isolated runtime directory.
   */
  computerUseCodexHome?: string;
  platform: string;
}
...
export async function resolveSkills(input: ResolveSkillsInput): Promise<ResolvedSkill[]> {
  ...
  const computerPath = await findComputerUseSkill({
    codexHome: input.computerUseCodexHome || input.codexHome,
