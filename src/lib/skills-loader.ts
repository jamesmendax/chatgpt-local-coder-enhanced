...
export async function resolveAllSkills(workspaceRoot: string): Promise<SkillSummary[]> {
  ...
  // Desktop account isolation intentionally points CODEX_HOME at a per-account
  // runtime.  Computer Use is an OpenAI-bundled host component, so discover it
  // from the trusted host Codex home supplied by the launcher instead.
  const computerUseCodexHome = process.env.HARNESS_COMPUTER_USE_CODEX_HOME?.trim()
