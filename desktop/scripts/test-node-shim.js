...
async function main() {
  ...
  // Keep verification on the documented non-secret fixture path.  Calling
  // mcpSpawnSpec without an override would exercise production first-run
  // token generation and leave an encrypted credential in temporary state.
  env.ADMIN_TOKEN = "EXAMPLE_ADMIN_TOKEN";
  const previousAdminToken = process.env.ADMIN_TOKEN;
  process.env.ADMIN_TOKEN = env.ADMIN_TOKEN;
  try {
    if (harness.mcpSpawnSpec({ mcpPort: 3999, adminPort: 3998, workspacePath: tmp, toolProfile: "slim" }).command !== process.execPath) {
