...
async function call(name, payload) {
  const res = await launcher[name](payload);
...
function showView(name) {
  ...
  const titles = {
    ...
    settings: ["Settings", "初始化、发布与偏好"],
  };
  if (titles[name]) {
    $("page-title").textContent = titles[name][0];
    $("page-subtitle").textContent = state.info ? `v${state.info.version} · ${titles[name][1]}` : titles[name][1];
  }
...
async function loadSkillCatalog() {
  state.skills = await call("getSkillCatalog");
...
async function saveSkills() {
  ...
  try {
    ...
    renderSkillCatalog();
    toast("外部 Skill 配置已保存。", "success", 5000);
  } catch (err) {
    toast(err.message, "error", 9000);
  } finally {
...
function renderStatus(s) {
  ...
  $("mcp-stale").textContent = m.staleBuild == null ? "—" : String(m.staleBuild);
  ...
  $("btn-stop-external").disabled = busy || !((m.external && m.externalRecognized) || (t.external && t.externalRecognized));
...
function bind() {
  ...
  launcher.on("status", renderStatus);
  launcher.on("log", (batch) => {
    const grouped = {};
    for (const entry of batch) (grouped[entry.name] ||= []).push(entry.line);
    for (const [name, lines] of Object.entries(grouped)) appendLogLines(name, lines);
  });
  launcher.on("notice", (n) => toast(n.message, n.level === "error" ? "error" : "info", 8000));
...
async function init() {
  ...
  try {
    ...
    state.config = await call("getConfig");
    $("page-subtitle").textContent = `v${state.info.version} · ${state.info.isPackaged ? "已打包" : "开发模式"}`;
    const logs = await call("getLogs");
    state.logs = logs;
    renderLog("mcp");
    renderStatus(await call("getStatus"));
    if (!state.info.encryptionAvailable) toast("警告：safeStorage 不可用，无法安全保存 Runtime API Key。", "error", 12000);
    showView(state.config.setupDone ? "dashboard" : "settings");
    if (!state.config.setupDone) showSettingsTab("setup");
  } catch (err) {
