...
The desktop launcher now supports **multiple named account profiles in one app**.
The original configuration is preserved as the default account; each additional
profile receives its own config/runtime/log/Skill data root, MCP/Admin/Tunnel
ports, workspace boundary and encrypted tunnel credential. Starting or stopping
the current profile does not stop another profile, and switching the UI only
changes which profile is being controlled. Cross-profile duplicate ports,
Tunnel IDs and overlapping workspaces are rejected. See
[`docs/MULTI_ACCOUNT_GUIDE.zh-CN.md`](docs/MULTI_ACCOUNT_GUIDE.zh-CN.md) for the
second-account workflow and security boundary. This is process/configuration
isolation under one Windows user, **not an OS security sandbox**.

```powershell
cd desktop
npm install
npm run test                 # migration, account isolation, real Electron UI, two-MCP-process checks
